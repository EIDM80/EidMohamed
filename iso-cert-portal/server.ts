...
async function startServer() {
  ...
  app.post("/api/referrals/join", async (req, res) => {
    ...
    const slugifyForCode = (n: string): string => n.toUpperCase().replace(/[^A-Z0-9]+/g, "").slice(0, 10) || "REF";
    ...
    if (dbError) {
      ...
      return res.status(500).json({ error: "Could not create your referral link. Please try again." });
    }

    const baseUrl = typeof req.headers.origin === "string" ? req.headers.origin : "";
